...
namespace WorkFlow_Seguros_Futuro
{
    public partial class progreso : System.Web.UI.Page
    {
        ...
        protected void Buscar_Click(object sender, DirectEventArgs e)
        {
            ...
            sql += "  select DISTINCT wf.correlativo,LI.Poliza_Maestra,wf.asegurado, wf.Estado,(wf.Puntos *0.01) Puntos,case when area ='RECEPCION' then DATEDIFF(DAY,convert(date,fecha_destino,101),convert(date,getdate(),101))  ";
            sql += "  when area ='EMISION' then DATEDIFF(DAY,convert(date,fecha_destino,101),convert(date,getdate(),101)) ";
            sql += "  when area ='ATENCION AL CLIENTE' then DATEDIFF(DAY,convert(date,fecha_destino,101),convert(date,f_rec_cliente,101))  ";
            sql += "  when Estado ='COMPLETADO' then DATEDIFF(DAY,convert(date,fecha_destino,101),convert(date,f_rec_cliente,101)) end as dias ";
            sql += "  from wf_seguimiento_95 wf ";
            sql += " LEFT JOIN LINKED.FUTUROSIS_DAT.dbo.vw_controldocumental_wf AS LI ON LI.ControlDocumental  = wf.correlativo COLLATE Modern_Spanish_CI_AS ";
            sql += " where cod_area=01 and Estado not in ('COMPLETADO','ATENCION AL CLIENTE') "; 
        ...
        protected void Indicador(object sender, DirectEventArgs e)
        {
            ...
            sql_detalle += " select wf.correlativo,paso, case when ej.fin is null  then DATEDIFF(DAY,convert(date,ej.inicio,101),convert(date,getdate(),101))  ";
            sql_detalle += " when inicio is null and fin is null then NULL ";
            sql_detalle += " else DATEDIFF(DAY,convert(date,ej.inicio,101),convert(date,ej.fin,101)) end as dias ";
            sql_detalle += " from wf_seguimiento_95 wf ";
            sql_detalle += " inner join dbo. ejecucion ej on  ej.correlativo=wf.correlativo ";
            sql_detalle += " where wf.correlativo =@correlativo ";
